public class AES {
    ...
    public String decrypt() {
        this.hexPlainText = this.message;
        /* no conversion to hex because the encrypted message is already in hex form.*/
 ...
 /* shifting rows according column */
    ...
    private String[][] generateIntermediateKeys(String[][] keyMatrix) {
        ...
        /* 4x4 matrix for 11 rounds i.e. 4 x 44 matrix*/
 /* add given key as it is in the keys matrix, because for first round the key is same.*/
        ...
        for (int j = 4; j < 44; j++) {
            if (j % 4 == 0) {
                ...
                String[] columnVector = extractColumnVector(keys, j - 1);
                /* consider it as a column vector [4x1]*/
    ...
    private String[] xorVector(String[] a, String[] b) {
        ...
        for (int i = 0; i < a.length; i++) {
            res[i] = intToHex(hexToInt(a[i]) ^ hexToInt(b[i]));
            /* hexString -> int then XOR then back to hexString*/
        }
        return res;
    }
    ...
    private String getSBoxSubstitute(String hex, final char[] sBox) {
        ...
        String repeated = new String(new char[2 - temp.length()]).replace("\0", "0");
    ...
    private String intToHex(int num) {
        ...
        String repeated = new String(new char[2 - hex.length()]).replace("\0", "0");
    ...
    private String stringToHex(String string) {
        ...
        String repeated = new String(new char[MAX_STRING_SIZE - string.length()]).replace("\0", " ");
